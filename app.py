...
# ------------------------------------------
with tabs[0]:
    col_h1, col_h2 = st.columns([3.2, 1.8], vertical_alignment="center")
    with col_h1:
        st.markdown(f"""
        <div style="display:flex; justify-content:space-between; align-items:center; padding: 2px 0;">
            <h3 style="margin:0; padding:0; font-size:1.15rem; font-weight:700; color:#F8FAFC; line-height:1.2;">🏦 Cash & Checking</h3>
            <span style="font-size:1.1rem; font-weight:800; color:#38BDF8; margin-left: 8px;">${total_cash:,.2f}</span>
        </div>
        """, unsafe_allow_html=True)
    with col_h2:
        ...
        with st.form("log_expense_form", clear_on_submit=True):
            ...
            if st.form_submit_button("Record Expense"):
                if amt is None or amt <= 0:
                    ...
                    try:
                        append_tx_to_sheet(row)
                        st.success(f"Saved ${amt:,.2f} expense to {selected_acc}!")
                        st.rerun()
                    except Exception as err:
                        st.error(f"Error: {err}")
        ...
        with st.form("log_income_form", clear_on_submit=True):
            ...
            if st.form_submit_button("Record Income"):
                if inc_amt is None or inc_amt <= 0:
                    ...
                    try:
                        append_tx_to_sheet(row)
                        st.success(f"Deposited ${inc_amt:,.2f} into {inc_acc}!")
                        st.rerun()
                    except Exception as err:
                        st.error(f"Error: {err}")
...
with tabs[2]:
    ...
    w_donut_labels, w_donut_values, w_donut_colors, w_donut_hovers = [], [], [], []
    for cat_name, budget_amt in WEEKLY_BUDGET_TARGETS.items():
        ...
        if spent_slice > 0:
            ...
            w_donut_hovers.append(f"<b>{cat_name}</b><br>Spent: ${spent_amt:.2f} /${budget_amt:.2f}")
        ...
        if unspent_slice > 0:
            ...
            w_donut_hovers.append(f"<b>{cat_name}</b><br>Remaining: ${unspent_slice:.2f} of${budget_amt:.2f}")
        ...
        if spent_amt > budget_amt:
            ...
            w_donut_hovers.append(f"<b>{cat_name} OVER</b><br>Over by: +${over_slice:.2f}")
    ...
    fig_week_donut = go.Figure(go.Pie(
        labels=w_donut_labels, values=w_donut_values, hole=0.55, sort=False, direction='clockwise',
        marker=dict(colors=w_donut_colors, line=dict(color='#0F172A', width=1.5)),
        hovertemplate="%{customdata}<extra></extra>", customdata=w_donut_hovers, textinfo='none'
    ))
    ...
    with m_col1:
        if st.button("◀", key="prev_month_btn", help="Previous Month"):
            ...
            st.rerun()
    with m_col2:
        st.markdown(f"<div style='text-align:center; font-weight:700; font-size:16px; color:#38BDF8; padding-top:8px;'>{month_start.strftime('%B %Y')}</div>", unsafe_allow_html=True)
    with m_col3:
    ...
    m_donut_labels, m_donut_values, m_donut_colors, m_donut_hovers = [], [], [], []
    for cat_name, w_base in WEEKLY_BUDGET_TARGETS.items():
        ...
        if spent_slice > 0:
            ...
            m_donut_hovers.append(f"<b>{cat_name}</b><br>Spent: ${spent_amt:.2f} /${m_cat_budget:.2f}")
        ...
        if unspent_slice > 0:
            ...
            m_donut_hovers.append(f"<b>{cat_name}</b><br>Remaining: ${unspent_slice:.2f} of${m_cat_budget:.2f}")
    ...
    fig_month_donut = go.Figure(go.Pie(
        labels=m_donut_labels, values=m_donut_values, hole=0.55, sort=False, direction='clockwise',
        marker=dict(colors=m_donut_colors, line=dict(color='#0F172A', width=1.5)),
        hovertemplate="%{customdata}<extra></extra>", customdata=m_donut_hovers, textinfo='none'
    ))
