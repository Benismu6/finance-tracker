...
def fetch_ai_insights_cached(net_cash, tot_cash, p_debt, b_debt, p_util, p_lim):
    try:
        ...
        pass
    return r"💡 **Key Next Steps:** Maintain utilization targets and monitor your liquidity toward the Baltimore home fund."

# ==========================================
# 5. APP TABS & INSTANT UI RENDERING (ACCOUNTS HUB AS DEFAULT)
# ==========================================
if "preset_account" not in st.session_state:
    st.session_state.preset_account = "Chase 1993 (Primary Daily)"

# Accounts & Credit Hub is now Tab 0 (Default Load Page)
tabs = st.tabs([
    "💳 Accounts & Credit Hub", 
    "⚡ Command Center", 
    "📊 Analytics & Charts", 
...
# ------------------------------------------
# TAB 0: ACCOUNTS & CREDIT HUB (DEFAULT LOAD PAGE)
# ------------------------------------------
with tabs[0]:
    ...
    st.caption(f"Overall Personal Util: **{personal_utilization:.2f}%** (${personal_cc_debt:,.2f} / ${personal_cc_limit:,.2f}).")
...
with tabs[1]:
    ...
    with tab_exp:
        with st.form("log_expense_form", clear_on_submit=True):
            ...
            if st.form_submit_button("Record Expense"):
                ...
                new_row_values = [
                    tx_id, date_str, clean_acc, "Expense", selected_cat, vendor,
                    float(amt), goal_tag, item_desc, "Mobile App Entry"
                ]
                try:
    ...
    with tab_inc:
        with st.form("log_income_form", clear_on_submit=True):
            ...
            inc_item = st.text_input("Income Memo (Optional)", placeholder="e.g. Pay period, Weekend boost", key="f_inc_item")
            ...
            if st.form_submit_button("Record Income"):
                ...
                new_row_values = [
                    tx_id, date_str, clean_inc_acc, "Income", inc_cat, inc_desc,
                    float(inc_amt), goal, inc_item, "Mobile App Entry"
                ]
                try:
                    append_tx_to_sheet(new_row_values)
    ...
    with tab_pay:
        with st.form("log_payment_form", clear_on_submit=True):
            ...
            if st.form_submit_button("Record CC Payment"):
                ...
                new_row_values = [
                    tx_id, date_str, target_card, "CC Payment", "CC Payment",
                    f"Paid from {clean_from}", float(pay_amt), "General Living", pay_item, "Mobile App Entry"
                ]
                try:
                    append_tx_to_sheet(new_row_values)
...
with tabs[2]:
    ...
    with st.expander("📅 Jump to Specific Date / Past Year", expanded=False):
        picked_date = st.date_input("Select any date:", value=st.session_state.selected_date, key="jump_calendar_picker")
    ...
    with w_col1:
        if st.button("◀", key="prev_week_btn"):
            ...
            st.rerun()
    with w_col2:
        st.markdown(f"<div style='text-align:center; font-weight:700; font-size:14px; color:#38BDF8; padding-top:8px;'>{week_start.strftime('%b %d')} – {week_end.strftime('%b %d, %Y')}</div>", unsafe_allow_html=True)
    with w_col3:
...
with tabs[4]:
    ...
    st.caption("Ask questions about your budget, credit card strategy, or home purchase goal.")
    ...
    if user_prompt := st.chat_input("Ask a question about your finances..."):
        ...
        system_context = f"""
        You are an elite personal financial advisor and real estate strategist.
        Live financial snapshot:
        - Total Cash: ${total_cash:,.2f}
        - Personal CC Debt: ${personal_cc_debt:,.2f} (Util: {personal_utilization:.2f}%)
        - Net Liquid Cash: ${net_liquid_cash:,.2f}
        - Home Goal: $26,500 target by March 1, 2027 (${total_cash:,.2f} saved).
        """
        ...
        with st.chat_message("assistant"):
            try:
                ...
                if api_key:
                    genai.configure(api_key=api_key)
                    model = genai.GenerativeModel("gemini-3.6-flash", system_identifier=system_context)
